...
export default task(
  "exectransaction",
  `Sends a tx to gnosisSafeProxy.execTransaction() on Rinkeby`
)
  .addPositionalParam(
  ...
  .setAction(async (taskArgs) => {
    try {
      ...
      if (!taskArgs.signatures) {
        taskArgs.signatures = `0x000000000000000000000000${userAddress.replace(
          "0x",
          ""
      ...
      const gnosisSafeProxy = await run("instantiateContract", {
        name: "IGnosisSafe",
        address: taskArgs.gnosissafeproxyaddress,
        write: true,
        signer: user,
      });
